...
// Third workflow. n-1 blocks waiting in the `processBlock` queue
...
module.exports = async function (processBlock, verifyBlock, processAddress) {
  try {
    ...
    processBlock.prefetch(1)
    processBlock.consume('processBlock', block)
  }
  catch(err) {
  ...
  async function block (msg) {
    ...
    try {
      ...
      let block = await axios.get('https://' + (process.env.NODE_ADDRESS || process.env.NODE_IP + ':' + process.env.NODE_PORT) + '/blocks/at/' + height, {
      ...
      // Update the block with the latest metadata
        ...
        if(+process.env.UPDATE_ADDRESS) {
          await processAddress.sendToQueue('processAddress', new Buffer(JSON.stringify(block.generator)), {
          correlationId: UUID()
        })
      }
        ...
        for (let tx of block.transactions) {
          ...
          try {
            ...
            if(+process.env.UPDATE_ADDRESS) {
              ...
              for (let tx of block.transactions) { uniqueprocessAddress.add(tx.recipient) }
              for (let tx of block.transactions) { uniqueprocessAddress.add(tx.sender) }
              ...
              for (let address of uniqueprocessAddress) {
                if(address) {
                  await processAddress.sendToQueue('addresses', new Buffer(JSON.stringify(address)), {
                    correlationId: UUID()
                  })
                }
              }
      ...
      if(+process.env.VERIFY_CACHE) {
        await verifyBlock.publish('delayed', 'block', new Buffer(JSON.stringify(block)), {
          correlationId: UUID(),
