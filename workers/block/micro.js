...
module.exports = async function (microBlock, verifyBlock, processAddress) {
  ...
  async function block (msg) {
    ...
    try {
      ...
      let block = await axios.get('https://' + process.env.NODE_ADDRESS + '/blocks/at/' + height, {
          ...
          try {
            ...
            if(+process.env.UPDATE_ADDRESS) {
              ...
              for (let address of uniqueprocessAddress) {
                if(address) {
                  await processAddress.sendToQueue('processAddress', new Buffer(JSON.stringify(address)), {
                    correlationId: UUID()
                  })
                }
              }
